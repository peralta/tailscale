...
var (
	...
	egressInterface = flag.String("egress-interface", "", "the interface on which traffic will be flowing - used for adjusting rate limits")
	egressDataLimit = flag.Int("egress-data-limit", 100*1024*1024/8, "the bandwidth in bytes/s DERP the server will try to stay under, only applies if egress-interface is set")
...
func main() {
	...
	if *egressInterface != "" && *egressDataLimit > 0 {
		s.StartEgressRateLimiter(*egressInterface, *egressDataLimit, *clientDataMin, *clientDataBurst)
	}
