...
func NewServer(privateKey key.NodePrivate, logf logger.Logf) *Server {
	...
	s := &Server{
		...
		clientDataBurst:      10 * 1024 * 1024, // 10Mb burst
	}
	atomic.StoreUint64(s.clientDataLimit, 8*1024*1024) // 8Mb/s steady-state ratelimit
	s.initMetacert()
...
func (s *Server) StartEgressRateLimiter(interfaceName string, egressDataLimit, clientDataMin, clientDataBurst int) {
	atomic.StoreUint64(s.clientDataLimit, uint64(egressDataLimit))
	s.clientDataBurst = clientDataBurst

	go func() {
		var (
			limiter = newEgressLimiter(interfaceName, egressDataLimit, clientDataMin)
