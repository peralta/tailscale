...
func TestLimiterLoopGradual(t *testing.T) {
	...
	if v := int(l.tick(1024*1024, now)); v < mb(150) || v > limit {
	...
	for i := 0; i < 60; i++ {
		now = now.Add(time.Second)
		l.tick(i*limit/60, now)
	}
	if v := int(l.tick(limit, now)); v > mb(100) || v < mb(1) {
	...
	if v := int(l.tick(limit, now)); v > mb(20) || v < mb(1) {
